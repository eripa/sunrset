...
module Sunrset
  class TimeAndSun
    ...
    def initialize longitude, latitude
      ...
      @date = Date.parse @time_data["localtime"].first
    ...
    def time_zone
      ...
      offset = "+#{offset}" if offset.to_i > 0
      offset
    end

    def sunrise
      DateTime.parse "#{@date.to_s} #{@sun_data["morning"].first["sunrise"].first}"
    end

    def sunset
      DateTime.parse "#{@date.to_s} #{@sun_data["evening"].first["sunset"].first}"
    end
  end
